...
volatile int servoPin = 2;      // the pin that the servo signal is attached to, needs to be a PWM pin.
int brightness = 150;
int close_pos = 2;
int open_pos = 120;
...
void setup()
{
  ...
  myservo.write(close_pos); // Initial position of the servo
...
void SetShutter(int val)
{
  ...
  {
    for (int angle = close_pos; angle <= open_pos; angle+=1) 
    {
      myservo.write (angle);
      delay (10);
    }     
  ...
  {
    for (int angle = open_pos; angle >= close_pos; angle-=1) 
    {
      myservo.write (angle);
      delay (10);
    }     
